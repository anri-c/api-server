...
from .routers import health_router, auth_router, items_router
...
def configure_service_exception_handlers(app: FastAPI) -> None:
    ...
    """
    try:
        from .services.auth_service import LineAuthError, JWTError
        ...
        async def service_exception_handler(request: Request, exc: Exception) -> JSONResponse:
            ...
            elif isinstance(exc, (ItemServiceError, ItemNotFoundServiceError, 
                                 ItemAccessDeniedServiceError, ItemValidationError)):
                api_exc = APIException(
                    message=exc.message,
                    status_code=exc.status_code,
                    error_code=type(exc).__name__.lower().replace("serviceerror", "_error").replace("error", "_error")
                )
            else:
                api_exc = APIException(
                    message=str(exc),
        ...
        app.add_exception_handler(ItemAccessDeniedServiceError, service_exception_handler)
