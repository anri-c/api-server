...
class UserServiceError(Exception):
    ...
    def __init__(self, message: str, status_code: int = 400, original_error: Optional[Exception] = None) -> None:
...
class UserService:
    ...
    async def get_user_by_id(self, user_id: int) -> Optional[UserResponse]:
        ...
        try:
            ...
            raise UserServiceError(
                f"Failed to get user: {e.message}",
                status_code=500,
                original_error=e
            ) from e
    
    async def get_user_by_line_id(self, line_user_id: str) -> Optional[UserResponse]:
    ...
    async def create_user_from_line_profile(self, profile: LineUserProfile) -> UserResponse:
        ...
        try:
            ...
            if existing_user:
                return existing_user
            raise UserServiceError(
                f"User creation failed: {e.message}",
                status_code=409,
                original_error=e
            ) from e
        except UserRepositoryError as e:
            raise UserServiceError(
                f"Failed to create user: {e.message}",
                status_code=500,
                original_error=e
            ) from e
    ...
    async def update_user(self, user_id: int, user_data: UserUpdate) -> Optional[UserResponse]:
        ...
        try:
            ...
            raise UserServiceError(
                f"Failed to update user: {e.message}",
                status_code=500,
                original_error=e
            ) from e
    ...
    async def delete_user(self, user_id: int) -> bool:
        ...
        try:
            ...
            raise UserServiceError(
                f"Failed to delete user: {e.message}",
                status_code=500,
                original_error=e
            ) from e
    ...
    async def get_all_users(self, limit: int = 100, offset: int = 0) -> List[UserResponse]:
        ...
        try:
            ...
            raise UserServiceError(
                f"Failed to get users: {e.message}",
                status_code=500,
                original_error=e
            ) from e
    ...
    async def get_user_count(self) -> int:
        ...
        try:
            ...
            raise UserServiceError(
                f"Failed to count users: {e.message}",
                status_code=500,
                original_error=e
            ) from e
    
    async def get_or_create_user_from_line_profile(self, profile: LineUserProfile) -> UserResponse:
