...
from ..logging_config import SecurityLoggingMixin, log_external_api_call, get_logger
...
class LineUserProfile(BaseModel):
    ...
    pictureUrl: Optional[str] = Field(default=None, description="Profile picture URL")
    statusMessage: Optional[str] = Field(default=None, description="User status message")
...
class AuthService(SecurityLoggingMixin):
    ...
    async def verify_line_access_token(self, access_token: str) -> LineTokenInfo:
        ...
        try:
            ...
            raise LineAuthError(
                f"Failed to connect to LINE API: {str(e)}",
                status_code=503
            ) from e
    ...
    async def get_line_user_profile(self, access_token: str) -> LineUserProfile:
        ...
        try:
            ...
            if isinstance(e, LineAuthError):
                raise
            raise LineAuthError(
                f"Unexpected error during profile retrieval: {str(e)}",
                status_code=500
            ) from e
    ...
    def create_jwt_token(self, user_id: int, line_user_id: str) -> str:
        ...
        try:
            ...
            raise JWTError(
                f"Failed to create JWT token: {str(e)}",
                status_code=500
            ) from e
    ...
    async def get_current_user_id(self, authorization: Optional[str]) -> int:
