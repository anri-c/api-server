...
class Item(ItemBase, table=True):
    ...
    user: Optional["User"] = Relationship(
        back_populates="items", sa_relationship_kwargs={"lazy": "select"}
    )
    ...
    __table_args__ = (
        ...
        Index(
            "idx_items_user_created", "user_id", "created_at"
        ),  # Composite index for user's items by date
    )
...
class ItemUpdate(SQLModel):
    ...
    name: str | None = Field(
        default=None, max_length=100, min_length=1, description="Updated item name"
    )
    description: str | None = Field(
        default=None, max_length=1000, description="Updated item description"
    )
