...
class ItemCreate(BaseModel):
    ...
    def validate_name(cls, v: str) -> str:
        ...
        if len(v.strip()) < 1:
            raise ValueError('Item name must contain at least 1 non-whitespace character')
...
class ItemListResponse(BaseModel):
    ...
    def validate_total_pages(cls, v: int, values: dict) -> int:
        ...
        if 'total' in values and 'page_size' in values:
            expected_pages = (values['total'] + values['page_size'] - 1) // values['page_size']
            if v != expected_pages:
...
class ItemListRequest(BaseModel):
    ...
    @validator('max_price')
    def validate_price_range(cls, v: Optional[Decimal], values: dict) -> Optional[Decimal]:
