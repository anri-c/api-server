...
from ..services.auth_service import AuthService, LineAuthError, JWTError
...
async def line_login_callback(
    ...
    try:
        ...
        raise HTTPException(
            ...
            detail=f"LINE authentication failed: {e.message}"
        ) from e
    except UserServiceError as e:
        raise HTTPException(
            status_code=e.status_code,
            detail=f"User service error: {e.message}"
        ) from e
    except JWTError as e:
        raise HTTPException(
            status_code=e.status_code,
            detail=f"Token generation failed: {e.message}"
        ) from e
