...
class TestItemService:
    ...
    async def test_create_item_database_error(
        ...
        item_service.user_repository = mock_user_repository
        mock_user_repository.get_by_id = AsyncMock(return_value=sample_user)
        mock_item_repository.create = Mock(side_effect=SQLAlchemyError("Database error"))
    ...
    async def test_get_items_for_user_with_price_range(
        ...
        # Setup mock
        item_service.item_repository = mock_item_repository
        mock_item_repository.get_user_items_by_price_range = Mock(return_value=[sample_item])
        mock_item_repository.count_for_user = Mock(return_value=1)
    ...
    @pytest.mark.asyncio
    async def test_get_items_for_user_invalid_request(
        self,
        item_service: ItemService
    ):
    ...
    async def test_update_item_not_found(
        ...
        # Setup mock
        item_service.item_repository = mock_item_repository
        mock_item_repository.update = Mock(side_effect=ItemNotFoundError("Item not found"))
    ...
    async def test_delete_item_access_denied(
        ...
        # Setup mock
        item_service.item_repository = mock_item_repository
        mock_item_repository.delete = Mock(side_effect=ItemAccessDeniedError("Access denied"))
    ...
    @pytest.mark.asyncio
    async def test_search_items_invalid_pagination(
        self,
        item_service: ItemService
    ):
...
class TestItemServiceValidation:
    ...
    def test_validate_item_create_data_description_too_long(self, item_service: ItemService):
