...
class TestAuthEndpoints:
    ...
    def test_line_callback_success_new_user(
        ...
        with patch("src.api_server.services.auth_service.AuthService.authenticate_line_user") as mock_auth:
            mock_auth.return_value = sample_line_profile
            
            with patch("src.api_server.services.user_service.UserService.get_or_create_user_from_line_profile") as mock_user_service:
                ...
                with patch("src.api_server.services.auth_service.AuthService.create_jwt_token") as mock_jwt:
                    ...
                    assert data["user"]["display_name"] == sample_line_profile.displayName
    ...
    def test_line_callback_invalid_token(self, client: TestClient):
        ...
        with patch("src.api_server.services.auth_service.AuthService.authenticate_line_user") as mock_auth:
            ...
            response = client.post("/api/auth/line/callback", json={
                "access_token": "invalid_line_access_token"
            })
    ...
    def test_line_callback_database_connection_error(
        ...
        with patch("src.api_server.services.auth_service.AuthService.authenticate_line_user") as mock_auth:
            ...
            with patch("src.api_server.services.user_service.UserService.get_or_create_user_from_line_profile") as mock_user_service:
                ...
                mock_user_service.side_effect = SQLAlchemyError("Database connection failed")
        ...
        with patch("src.api_server.services.auth_service.AuthService.authenticate_line_user") as mock_auth:
            ...
            with patch("src.api_server.services.user_service.UserService.get_or_create_user_from_line_profile") as mock_user_service:
                ...
                with patch("src.api_server.services.auth_service.AuthService.create_jwt_token") as mock_jwt:
                    ...
                    auth_response = client.post("/api/auth/line/callback", json={
                        "access_token": "valid_line_access_token"
                    })
            ...
            with patch("src.api_server.services.auth_service.AuthService.authenticate_line_user") as mock_auth:
                ...
                with patch("src.api_server.services.user_service.UserService.get_or_create_user_from_line_profile") as mock_user_service:
                    ...
                    with patch("src.api_server.services.auth_service.AuthService.create_jwt_token") as mock_jwt:
                        ...
                        return client.post("/api/auth/line/callback", json={
                            "access_token": "valid_line_access_token"
                        })
